...
class CMAC(object):
    ...
    def quantize(self, vector):  
        ...
        for i in range(self.nlevels):  
            ...
            point.append(i)
            # print i, quantized, quantized -i,  (quantized - i) % self.nlevels, quantized - (quantized - i) % self.nlevels, point
    ...
    def response(self, vector, response, quantized = False):   #n-dim vector(X): [x1, x2 ,...] , response(Y)
        """
        Train the CMAC.
        """
        ...
        coords = None
        if quantized == False:
            coords = self.quantize(vector)    #converts a 2dim value to a 32 dim value
        else:
            coords = vector
...
def test(name):
    ...
    if name == 'sin':
        ...
        cmac = CMAC(32, .1, 0.01)    #nlevels(2^n), quantization, beta(like a learning rate of sorts)
        points = uniform(low=0,high=2*pi,size=(10000,2))
        responses = sin(points[:,0]) + cos(points[:,1])

        errors = []
        for i,(point,response) in enumerate(zip(points,responses)):
        ...
        ax1 = Axes3D(fig1)
        ax1.scatter(points[:,0], points[:,1], responses,color='red')
        ...
        ax2 = Axes3D(fig2)
        ax2.scatter(points[:,0], points[:,1], predictions, color='green')
        print 'Plotting ...'
        # print len(cmac)
        # pylab.plot(errors)
        pylab.show()
